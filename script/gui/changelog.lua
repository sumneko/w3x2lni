...
return function(window_, canvas_)
    ...
    version('1.7', function()
        ...
        log('FIX', '修正多行字符串会使slk错误的问题')
        log('FIX', '修正单位ui名可能失效的问题', '用于对战函数中给市场添加物品，以及检测是否有主基地')
        log('FIX', '修正obj格式下长文本导致的崩溃问题')
    end)
    version('1.6', function()
        log('NEW', 'slk时可以选择是否优化装饰物', '优化装饰物后地形变化不会导致卡顿，但是玩下一张地图前需要重启魔兽，否则装饰物会显示不正常')
        log('CHG', 'slk文件的key不再无视大小写')
        log('FIX', '修正不支持中文路径的问题')
        log('FIX', '修正技能数据版本不正确的问题')
        log('FIX', '修正部分技能目标允许错误的问题', '默认目标允许为空的技能')
    end)
