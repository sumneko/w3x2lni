...
local function get_typedefine(w2l, type)
    if not typedefine then
        typedefine = w2l:parse_lni(io.load(w2l.root / w2l.defined / 'typedefine.ini'))
    end
...
local function create_metadata(w2l, type, metadata)
    ...
    local tbl = slk(io.load(w2l.root / w2l.meta / w2l.info.metadata[type]))
    tbl.Ytip = nil
    local has_index = {}
...
return function(w2l)
    ...
    io.save(w2l.root / w2l.defined / 'metadata.ini', stringify_ex(metadata))
end
