...
function w2l:backend_obj(type, ...)
    ...
    if type == 'ability' then
        assert(buf:find('AHtb-000', 1, true))
        assert(buf:find('AHtb.AAA', 1, true))
        assert(buf:find('AHtbAAA]', 1, true))
        ok1 = buf
    end
    return buf
end
...
assert(objs['AAA]'].anam[1] == slk.ability['AAA]'].name)
assert(slk.ability['AHtb']._mark == true)
